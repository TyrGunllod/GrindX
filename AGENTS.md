...
- **Mensageiro**: `app/mensagens/` (pacote top-level) + `modules/mensagens/` + `shared/mensagensWidget.js`. Categorias `SISTEMA`/`AVISO` admin-only (remetente NULL = sistema, sem resposta); `POST /v1/mensagens/broadcast` cria uma por usuário ativo; lista por participante (remetente ou destinatário) com `nao_lida` calculado no servidor; arquivamento só pelo destinatário da raiz (thread inteira). Badge/balão do mascote via `mensagensWidget.js` (polling 30s + `visibilitychange`/`focus`); comunicação iframe→pai via `shared/notificationBridge.js` (`grindx:mensagens-atualizar` / `grindx:navegar`). `dashboard.html` inclui `notificationBridge.js` e `mensagensWidget.js` (ambos no precache do `sw.js` — bumpar `CACHE` ao alterar)

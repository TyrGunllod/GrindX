...
from app.modules.custo.schemas.custo_produto import CustoProdutoResponse, CustoProdutoRangeResponse
...
try:
    ...
    try:
        ...
        async def _auth_dependency(): pass

try:
    from app.modules.custo.services.custo_produto_pdf_service import gerar_pdf, gerar_pdf_range
...
def calcular_range(inicial: str = Query(...), final: str = Query(...),
...
def pdf_range(inicial: str = Query(...), final: str = Query(...),
              service: CustoProdutoService = Depends(get_custo_produto_service),
              auth=Depends(_auth_dependency)):
    if not _pdf_disponivel:
        raise HTTPException(status_code=500, detail="xhtml2pdf não instalado. Execute: pip install xhtml2pdf")
    result = service.calcular_range(inicial, final)
    pdf_bytes = gerar_pdf_range(inicial, final, result.produtos)
    return Response(pdf_bytes, media_type="application/pdf",
                    headers={"Content-Disposition": f"attachment; filename={inicial}-{final}_resumido.pdf"})


@router.get("/{codigo}")
def calcular(codigo: str, service: CustoProdutoService = Depends(get_custo_produto_service),
             auth=Depends(_auth_dependency)):
    return service.calcular(codigo)


@router.get("/{codigo}/pdf")
def pdf_single(codigo: str,
               service: CustoProdutoService = Depends(get_custo_produto_service),
               auth=Depends(_auth_dependency)):
    if not _pdf_disponivel:
        raise HTTPException(status_code=500, detail="xhtml2pdf não instalado. Execute: pip install xhtml2pdf")

"""
Testes de integração para o endpoint de health check da api-sqlserver.

Cobre cenários de saúde (healthy) e degradação (degraded)
quando o banco de dados SQL Server está indisponível.
"""
...
class TestHealthCheckHealthy:
    ...
    def test_health_database_dict_with_sqlserver_key(
        self, client: TestClient, db_session: Session
    ):
        """GET /health retorna dict 'database' com chave 'sqlserver'."""
        response = client.get("/health")
        data = response.json()
        assert "database" in data
        assert isinstance(data["database"], dict)
        assert "sqlserver" in data["database"]
        assert data["database"]["sqlserver"] == "connected"
...
class TestHealthCheckDegraded:
    ...
    def test_health_database_dict_disconnected_on_failure(self, client: TestClient):
        ...
        with patch(
            ...
            assert isinstance(data["database"], dict)
            assert "sqlserver" in data["database"]
            assert data["database"]["sqlserver"] == "disconnected"

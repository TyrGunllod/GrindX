...
def copy_frontend(import_dir: Path, module_name: str, force: bool) -> None:
    ...
    for item in src.iterdir():
        if item.name == "shared":
            ...
            logger.info(
                "Ignorando frontend/shared/ (compatibilidade) — já existe no monorepo"
            )
            continue
        if item.is_dir():
...
def import_module(
    ...
    try:
        ...
        if not dry_run:
            ...
            manifest_src = import_dir / "module.json"
            if manifest_src.exists():
                api_dir = (
                    _get_api_dir()
                    if target_api == "postgres"
                    else _get_sqlserver_api_dir()
                )
                manifest_dest = (
        ...
        if not dry_run and not is_frontend_only:
            if target_api == "sqlserver":
